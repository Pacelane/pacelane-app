...
export const ContentSuggestions: React.FC<ContentSuggestionsProps> = ({
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('content_suggestions').select('id, title, description, suggested_outline').eq('user_id', user.id).eq('is_active', true).order('created_at', {
        ascending: false
      }).limit(3);
  ...
  const generateNewSuggestions = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-content-suggestions', {
        headers: {
  ...
  return <div className="w-full max-w-2xl mx-auto p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <Button variant="ghost" size="sm" className="text-gray-600 hover:text-gray-900 text-xs" onClick={generateNewSuggestions} disabled={generating}>
        ...
        <CardContent className="p-0">
          ...
          <div className="divide-y divide-gray-100">
            {loading ? <div className="p-8 text-center text-gray-500">
                ...
                Loading suggestions...
              </div> : suggestions.length === 0 ? <div className="p-8 text-center text-gray-500">
                <Sparkles className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                <p>No suggestions available. Click "New Suggestions" to generate some!</p>
              </div> : suggestions.map(suggestion => <div key={suggestion.id} className="flex items-center justify-between p-3 hover:bg-gray-50 transition-colors">
                  <div className="flex-1 pr-3">
                    <p className="text-gray-800 font-medium text-sm">
                      "{suggestion.title}"
                    </p>
                    {suggestion.description && <p className="text-xs text-gray-500 mt-1">
                        {suggestion.description}
                      </p>}
                  </div>
                  <Button variant="outline" size="sm" onClick={() => onWriteContent(suggestion)} className="shrink-0 text-gray-700 border-gray-300 hover:bg-gray-100 text-xs h-7 px-2">Post This</Button>
                </div>)}
          </div>
        </CardContent>
      </Card>
